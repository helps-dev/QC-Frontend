...
import { ChevronDown, ArrowDownUp, Info, BarChart2 } from '../../components/Icons3D'
    ...
    <div className="w-full max-w-[420px] bg-[#1a1a2e]/90 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl shadow-purple-900/20 p-5">
          ...
          <button
            ...
            className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${
              isChartVisible 
                ...
                : 'bg-[#252542] border border-white/5 text-gray-400 hover:text-white'
            }`}
          >
      ...
      <div className="space-y-3">
        ...
        <div className="bg-[#12121a] rounded-2xl p-4 border border-white/5">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              <button onClick={setMaxAmount} className="text-purple-400 hover:text-purple-300 font-semibold">MAX</button>
            </div>
          </div>
        ...
        <div className="flex justify-center -my-1 relative z-10">
          <button 
            ...
            className="w-10 h-10 bg-[#1a1a2e] border-4 border-[#0d0d15] hover:border-purple-500/30 rounded-xl flex items-center justify-center transition-all group"
          >
            <ArrowDownUp className="w-4 h-4 text-purple-400 group-hover:rotate-180 transition-all duration-300" />
          </button>
        </div>
        ...
        {isWrapOrUnwrap && amountIn && parseFloat(amountIn) > 0 && (
          <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-3 text-sm">
            <div className="flex items-center gap-2 text-purple-400">
              ...
              <span>{isWrapOperation ? 'Wrapping (1:1 ratio)' : 'Unwrapping (1:1 ratio)'}</span>
            </div>
          </div>
        )}
        ...
        {(!amountIn || parseFloat(amountIn) === 0) && (
          <button disabled className="w-full py-4 bg-[#252542]/50 rounded-2xl font-semibold text-gray-500 cursor-not-allowed border border-white/5">
            Enter Amount
          </button>
        )}
        ...
        {amountIn && parseFloat(amountIn) > 0 && (
          <div className="space-y-2">
            {!isConnected ? (
              ...
              >
                {isPending || isConfirming ? 'Approving...' : `Approve ${tokenIn.symbol}`}
              </button>
            ) : (
              <button
                onClick={handleSwap}
              ...
              >
                {isPending || isConfirming ? 'Processing...' : getButtonText()}
              </button>
            )}
          </div>
        )}
      ...
      {showTokenModal && (
        <TokenModal
          ...
          onImport={() => { setShowTokenModal(null); setShowImportModal(true) }}
        />
      )}

      {showImportModal && (
        <TokenImportModal onClose={() => setShowImportModal(false)} onTokenImported={handleTokenImported} />
      )}
    </div>
  )
}
