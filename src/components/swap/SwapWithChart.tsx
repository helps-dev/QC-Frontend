...
// Default pair to show when no specific pair is found
const DEFAULT_PAIR_ADDRESS = '0xcf4dc3db3223ee91ff52da4e110ba8abfb943843' // WMON/QUICK
...
export function SwapWithChart() {
  ...
  const [pairAddress, setPairAddress] = useState<string>(DEFAULT_PAIR_ADDRESS)
  ...
  useEffect(() => {
    const findPairAddress = async () => {
      if (!swapTokenIn || !swapTokenOut) {
        setPairAddress(DEFAULT_PAIR_ADDRESS)
        setPairNotFound(false)
        return
      }
      
      // Get actual token addresses (convert native MON to WMON)
      ...
      try {
        // Strategy 1: Search by token address (more accurate)
        ...
        if (searchRef.current !== searchKey) return // Abort if tokens changed
        
        // Filter for Monad chain and find matching pair
        const monadPairs1 = data1.pairs?.filter((p: DexScreenerPair) => p.chainId === 'monad') || []
        matchingPair = monadPairs1.find((pair: DexScreenerPair) => {
        ...
        if (!matchingPair) {
          ...
          const monadPairs2 = data2.pairs?.filter((p: DexScreenerPair) => p.chainId === 'monad') || []
          matchingPair = monadPairs2.find((pair: DexScreenerPair) => {
            const baseAddr = pair.baseToken.address.toLowerCase()
            const quoteAddr = pair.quoteToken.address.toLowerCase()
            const inAddr = tokenInAddr.toLowerCase()
            const outAddr = tokenOutAddr.toLowerCase()
        ...
        if (!matchingPair) {
          ...
          const monadPairs3 = data3.pairs?.filter((p: DexScreenerPair) => p.chainId === 'monad') || []
          matchingPair = monadPairs3.find((pair: DexScreenerPair) => {
            const baseAddr = pair.baseToken.address.toLowerCase()
            const quoteAddr = pair.quoteToken.address.toLowerCase()
            const inAddr = tokenInAddr.toLowerCase()
            const outAddr = tokenOutAddr.toLowerCase()
        ...
        if (matchingPair) {
          ...
          // Keep showing the last valid pair or default
  ...
  const fetchPairInfo = useCallback(async () => {
    ...
    try {
      const response = await fetch(`https://api.dexscreener.com/latest/dex/pairs/monad/${pairAddress}`)
  ...
  // Mobile/Tablet: Chart below swap (vertical layout)
  // Desktop: Chart beside swap (horizontal layout)
              ...
              <div className={isVerticalLayout ? 'w-full' : 'min-w-[350px] lg:min-w-[400px]'}>
                ...
                <div className="glass-card p-3 sm:p-4 rounded-xl">
                  ...
                  <div className="flex items-center justify-between gap-2 mb-3">
                    ...
                    {/* External Link */}
                    <a 
                      href={`https://dexscreener.com/monad/${pairAddress}`} 
                      target="_blank" 
                  ...
                  >
                    ...
                    <iframe
                      key={pairAddress} // Force re-render when pair changes
                      src={`https://dexscreener.com/monad/${pairAddress}?embed=1&theme=dark&trades=0&info=0`}
